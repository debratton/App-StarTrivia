...
class PersonApi {
    ...
    // Web Requests with AlmoFire and Codable
    ...
    /*
    // Web Request with AlmoFire and SwiftyJason
    func getRandomPersonAlamo(id: Int, completion: @escaping PersonResponseCompletion) {
        guard let url = URL(string: "\(PERSON_URL)\(id)") else { return }
        Alamofire.request(url).responseJSON { (response) in
            if let error = response.result.error {
...
*/
    // CODED MANUALLY WITHOUT COCOAPODS
    // Web Request with URL Session (1)
    ...
    //private func parsePersonSwifty(json: [String: Any]) -> Person {
    ...
    // Without using cocoapods SwiftyJSON
